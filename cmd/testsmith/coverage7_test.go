...
// testsmith coverage backfill / AC: runInit falls back to default TestRoot/FixtureDir when language has no language-specific config
...
// testsmith coverage backfill / AC: runMigrate logs error and counts failure when a test file is unreadable
...
// testsmith coverage backfill / AC: pruneOne reports unused fixtures and prints dry-run summary when fixtures exist but are not imported
...
// testsmith coverage backfill / AC: pruneOne deletes unused fixture files when isDryRun=false
...
// testsmith coverage backfill / AC: runInit prints dry-run plan for languages with a fixture directory
func TestRunInit_Python_DryRun(t *testing.T) {
	...
	// dryRun=true → .testsmith.yaml should NOT be created.
	if _, err := os.Stat(filepath.Join(dir, ".testsmith.yaml")); !os.IsNotExist(err) {
		t.Error("runInit dry-run must not create .testsmith.yaml")
	}
}

// ── processOne analyze error path (generate.go:420) ──────────────────────────

// testsmith coverage backfill / AC: processOne returns a fileResult with err set when AnalyzeFile fails
