...
func TestVersion(t *testing.T) {
	...
	if !strings.HasPrefix(out, "testsmith ") {
		t.Errorf("version output = %q, want prefix 'testsmith '", out)
	}
}
...
func TestConfigShow_WithFile(t *testing.T) {
	...
	if !strings.Contains(out, "mytests/") {
		t.Errorf("config show did not reflect .testsmith.yaml:\n%s", out)
	}
}
...
func TestInit_CreatesConfigFile(t *testing.T) {
	...
	if err != nil {
		t.Fatalf(".testsmith.yaml not created: %v", err)
	}
	body := string(content)
...
func TestInit_AlreadyExists_Skips(t *testing.T) {
	...
	preserved, _ := os.ReadFile(filepath.Join(dir, ".testsmith.yaml"))
	if string(preserved) != existing {
		t.Errorf("init overwrote existing .testsmith.yaml")
	}
}
