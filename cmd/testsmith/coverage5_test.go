...
// testsmith coverage backfill / AC: completion bash generates valid bash completion to stdout
...
// testsmith coverage backfill / AC: completion zsh generates valid zsh completion to stdout
...
// testsmith coverage backfill / AC: completion fish generates valid fish completion to stdout
...
// testsmith coverage backfill / AC: completion powershell generates valid PS completion to stdout
...
// testsmith coverage backfill / AC: runGenerate prints language/root/workers header when verbose=true
...
// testsmith coverage backfill / AC: runGenerate discovers files in --path directory when pathFlag is set
...
// testsmith coverage backfill / AC: runGenerate prints "No files found" when pathFlag points to empty directory
...
// testsmith coverage backfill / AC: runMigrate logs "unchanged" when migrator produces identical output and verbose=true
...
// testsmith coverage backfill / AC: newLearnCmd registers exactly one RunE handler
...
// testsmith coverage backfill / AC: pruneOne respects the isDryRun flag when printing dry-run summary
...
// testsmith coverage backfill / AC: runGenerate passes overwrite flag through to GenerateOpts
...
// testsmith coverage backfill / AC: collectValidationResults logs error and continues when a file cannot be read
...
// testsmith coverage backfill / AC: runValidate returns error when validation finds errors in files
...
// testsmith coverage backfill / AC: runGraph prints language/root when verbose=true
