...
// When GO_TESTSMITH_SUBPROCESS=1 is set, it reconstructs os.Args from
// GO_TESTSMITH_ARGS and drives the cobra command tree in-process.
//
...
func TestSubprocessMain(t *testing.T) {
	...
	rawArgs := os.Getenv("GO_TESTSMITH_ARGS")
	if rawArgs != "" {
		os.Args = append([]string{"testsmith"}, strings.Split(rawArgs, "\x1e")...)
	}
