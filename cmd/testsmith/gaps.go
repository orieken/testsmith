...
Examples:
  ...
  testsmith gaps --format json
  testsmith gaps --format junit --output results.xml
  testsmith gaps --top 10
  testsmith gaps --check
  testsmith gaps --workspace api`,
	...
	cmd.Flags().StringVar(&output, "output", "", "output file (default: testsmith_coverage_report.md for markdown, stdout for json/junit)")
...
func writeReport(gaps []domain.CoverageGap, totalSources, top int, output, root, format string) error {
	...
	dest := output
	if dest == "" {
		dest = "testsmith_coverage_report.md"
	}
