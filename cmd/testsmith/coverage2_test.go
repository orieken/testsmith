...
// testsmith coverage backfill / AC: buildDepIndex returns a non-nil module map for a minimal Go workspace
...
// testsmith coverage backfill / AC: printUsageReport emits the summary string from usageSummaryReporter
...
// testsmith coverage backfill / AC: printUsageReport is silent when the summary string is empty
...
// testsmith coverage backfill / AC: printUsageReport returns immediately when bg is nil
...
// testsmith coverage backfill / AC: resolveWorkspaceDriver auto-detects Go when workspace has no explicit language
...
// testsmith coverage backfill / AC: resolveWorkspaceDriver returns error when auto-detection fails (empty directory)
...
// testsmith coverage backfill / AC: validateRoot outputs JSON when format=json
...
// testsmith coverage backfill / AC: validateRoot outputs JUnit XML when format=junit
...
// testsmith coverage backfill / AC: printValidationText logs clean files when verbose=true
...
// testsmith coverage backfill / AC: printValidationText reports files with issues
...
// ── workspace branch for run* functions via .testsmith.yaml ──────────────────

// setupWorkspaceDir creates a temp directory structure with .testsmith.yaml
...
func setupWorkspaceDir(t *testing.T) (cwd string) {
	...
	// Write .testsmith.yaml pointing to the workspace.
	cfg := "workspaces:\n  - name: svc\n    path: svc\n    language: go\n"
...
// testsmith coverage backfill / AC: runGaps takes the workspace code path when .testsmith.yaml has workspaces
...
// testsmith coverage backfill / AC: runGraph takes the workspace code path when .testsmith.yaml has workspaces
...
// testsmith coverage backfill / AC: runPrune takes the workspace code path when .testsmith.yaml has workspaces
...
// testsmith coverage backfill / AC: runValidate takes the workspace code path when .testsmith.yaml has workspaces
...
// testsmith coverage backfill / AC: runMigrate with dryRun=true migrates jest to vitest without writing
...
// testsmith coverage backfill / AC: runMigrate restricts search to --path directory when pathFlag is set
...
// testsmith coverage backfill / AC: runInit without langHint auto-detects Go from go.mod
func TestRunInit_AutoDetectGo(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(dir, ".testsmith.yaml")); err != nil {
		t.Error(".testsmith.yaml not created after auto-detect")
	}
}

// ── runValidate verbose path (validate.go:129) ────────────────────────────────

// testsmith coverage backfill / AC: validateRoot prints Language/Adapter line when verbose=true and format=text
...
// testsmith coverage backfill / AC: collectGaps succeeds when given a valid Go project root
...
// testsmith coverage backfill / AC: fixtureRootFor returns empty string when FixtureDir is empty
...
// testsmith coverage backfill / AC: fixtureRootFor joins root and fixture dir when FixtureDir is set
...
// testsmith coverage backfill / AC: relPath returns a relative path when root is a proper prefix of path
...
// testsmith coverage backfill / AC: relPath falls back to the base name when relative path cannot be computed
...
// testsmith coverage backfill / AC: writeGraphReport prints content and skips file creation in dry-run mode
...
// testsmith coverage backfill / AC: runGaps prints language/root when verbose=true
...
// testsmith coverage backfill / AC: runInit returns error when no langHint is given and no project is detected
...
// testsmith coverage backfill / AC: runMigrate prints verbose header when verbose=true
