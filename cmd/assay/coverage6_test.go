...
// ── runInit .assay.yaml already-exists branch (init.go:107) ───────────────

// assay coverage backfill / AC: runInit skips and returns nil when .assay.yaml already exists
func TestRunInit_ConfigYAMLAlreadyExists(t *testing.T) {
	...
	mkFile(t, filepath.Join(dir, ".assay.yaml"), "language: go\n")

	if err := runInit("go", false); err != nil {
		t.Fatalf("runInit config exists: %v", err)
	}
	// Content must be unchanged.
	b, _ := os.ReadFile(filepath.Join(dir, ".assay.yaml"))
	if string(b) != "language: go\n" {
		t.Error("runInit must not overwrite an existing .assay.yaml")
	}
}

// ── runMigrate auto-detect error path (migrate.go:70) ────────────────────────

// assay coverage backfill / AC: runMigrate returns error when auto-detection fails in a directory with no project markers
...
// assay coverage backfill / AC: runValidate returns error when validation finds SeverityError issues
...
// assay coverage backfill / AC: runGenerateWorkspaces skips workspaces that do not match wsFilter
...
// assay coverage backfill / AC: pruneOne returns error when pipeline.DiscoverAndAnalyzeAll fails (bad root)
...
// assay coverage backfill / AC: runGapsWorkspaces skips workspaces not matching wsFilter
...
// assay coverage backfill / AC: runGraph workspace branch filters correctly via wsFilter parameter
...
// assay coverage backfill / AC: runPrune workspace branch filters correctly via wsFilter parameter
...
// assay coverage backfill / AC: runValidateWorkspaces skips workspaces that do not match wsFilter
...
// assay coverage backfill / AC: runGenerate returns error when called with no args, all=false, pathFlag=""
...
// assay coverage backfill / AC: runInit creates both testRoot and fixtureDir when the language defines a fixture directory
...
// assay coverage backfill / AC: runMigrate auto-detects TypeScript driver when langFlag is empty and package.json exists
...
// assay coverage backfill / AC: validateRoot logs and returns 0 when no adapter is selected for the driver
