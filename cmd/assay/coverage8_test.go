...
// assay coverage backfill / AC: completion RunE returns nil when shell argument is not a recognised value
...
// assay coverage backfill / AC: newLearnCmd.RunE delegates to runLearn and returns its error
...
// assay coverage backfill / AC: runLearn returns error when the given file is empty
...
// assay coverage backfill / AC: parseLearnResponse returns empty slug when the response lacks the filename header
...
// assay coverage backfill / AC: parseLearnResponse extracts slug and body when the response has the filename header (lowercase)
...
// assay coverage backfill / AC: slugFromPath converts a nested file path to a kebab-case slug
...
// assay coverage backfill / AC: walkTestFiles returns files after skipping dot-prefixed directories
...
// assay coverage backfill / AC: runMigrate prints "No test files found" when the TypeScript project has no test files
...
// assay coverage backfill / AC: divider with label returns a non-empty separator string containing the label
...
// assay coverage backfill / AC: divider with empty string returns a pure separator without label text
...
// assay coverage backfill / AC: printHeader does not panic and writes output
...
// assay coverage backfill / AC: adapterIndex returns -1 when target adapter is not in the list
...
// assay coverage backfill / AC: parseChoice returns -1 for non-numeric input
...
// assay coverage backfill / AC: parseChoice returns -1 for out-of-range input
...
// assay coverage backfill / AC: parseChoice returns valid zero-based index for valid input
