...
// Issue reference: assay coverage backfill
...
// assay coverage backfill / AC: prompt returns user-supplied text when input is non-empty
...
// assay coverage backfill / AC: prompt returns default when the user presses Enter without typing
...
// assay coverage backfill / AC: prompt returns default on EOF (e.g. piped empty stdin)
...
// assay coverage backfill / AC: prompt trims leading/trailing whitespace from user input
...
// assay coverage backfill / AC: pickLanguage with yes=true bypasses the prompt and returns the first language
...
// assay coverage backfill / AC: pickLanguage with explicit "1" input selects the first language
...
// assay coverage backfill / AC: pickLanguage with invalid choice "0" falls back to index 0
...
// assay coverage backfill / AC: issueIndicator returns ✗ when errors > 0
...
// assay coverage backfill / AC: countBySeverity correctly tallies errors, warnings, and ignores info
...
// assay coverage backfill / AC: listAvailableMigrations returns "(none)" for Go which has no text-substitution rules
...
// assay coverage backfill / AC: listAvailableMigrations returns migration rules for TypeScript (jest→vitest)
...
// assay coverage backfill / AC: buildBodyGen returns (nil, nil) when llmFlag is false — existing behaviour confirmed
...
// assay coverage backfill / AC: buildBodyGen propagates error from factory when API key is missing
...
// assay coverage backfill / AC: buildBodyGen returns non-nil generator for ollama (no API key required)
...
// assay coverage backfill / AC: printCacheStats is silent when verbose=false
...
// assay coverage backfill / AC: printCacheStats is silent when bg does not implement cacheStatsReporter
...
// assay coverage backfill / AC: printCacheStats emits cache stats when bg implements cacheStatsReporter
...
// assay coverage backfill / AC: writeReport prints JSON to stdout when format=json and output is empty
...
// assay coverage backfill / AC: writeReport prints JUnit XML to stdout when format=junit and output is empty
...
// assay coverage backfill / AC: writeReport prints the report when dryRun=true (does not write file)
...
// assay coverage backfill / AC: writeReport applies the --top limit before rendering
...
// assay coverage backfill / AC: writeAgents creates all three bundled agent files under .claude/agents/
...
// assay coverage backfill / AC: writeAgents skips files that already exist on a second call
...
// assay coverage backfill / AC: writeAgents respects dryRun and does not write agent file content
...
// assay coverage backfill / AC: runInit prints "already exists — skipping" when .assay/patterns/ already exists
func TestRunInit_PatternsDirAlreadyExists(t *testing.T) {
	...
	patternsDir := filepath.Join(dir, ".assay", "patterns")
	...
	// .assay.yaml must still be written.
	if _, err := os.Stat(filepath.Join(dir, ".assay.yaml")); err != nil {
		t.Error(".assay.yaml should have been created")
	}
}
...
func TestRunInit_ASSAYMDAlreadyExists(t *testing.T) {
	...
	// Pre-create ASSAY.md so runInit hits the "already exists" branch.
	knowledgePath := filepath.Join(dir, "ASSAY.md")
	if err := os.WriteFile(knowledgePath, []byte("# existing"), 0o644); err != nil {
		t.Fatal(err)
	}

	if err := runInit("go", false); err != nil {
		t.Fatalf("runInit with pre-existing ASSAY.md: %v", err)
	}

	// ASSAY.md must be unchanged.
	data, _ := os.ReadFile(knowledgePath)
	if string(data) != "# existing" {
		t.Error("runInit overwrote pre-existing ASSAY.md")
	}
}

// assay coverage backfill / AC: runInit with withAgents=true creates .claude/agents/ files
...
// assay coverage backfill / AC: runInit without langHint and in a directory with no project markers returns error
...
// assay coverage backfill / AC: runGapsWorkspaces processes a single Go workspace and writes a report
...
// assay coverage backfill / AC: runGapsWorkspaces skips workspaces that do not match wsFilter
...
// assay coverage backfill / AC: runGapsWorkspaces returns error when check=true and coverage gaps exist
...
// assay coverage backfill / AC: runGapsWorkspaces continues after a workspace that fails to resolve
...
// assay coverage backfill / AC: runGraphWorkspaces processes all workspaces and writes a combined report
...
// assay coverage backfill / AC: runGraphWorkspaces respects the wsFilter and skips non-matching workspaces
...
// assay coverage backfill / AC: runPruneWorkspaces iterates all workspaces and returns nil when no fixtures exist
...
// assay coverage backfill / AC: runPruneWorkspaces skips workspaces that do not match wsFilter
...
// assay coverage backfill / AC: runGaps returns error when check=true and gaps are found
...
// assay coverage backfill / AC: runGaps with json format and no output arg prints to stdout
