...
When workspaces are configured in .assay.yaml, each workspace
produces a labelled section in the same report.

Examples:
  assay graph
  assay graph --output deps.md
  assay graph --workspace api`,
		RunE: func(cmd *cobra.Command, args []string) error {
			return runGraph(output, workspace)
		},
	}

	cmd.Flags().StringVar(&output, "output", "assay_graph.md", "output Markdown file")
...
func runGraph(output, wsFilter string) error {
	...
	return writeGraphReport("# Assay Dependency Graph\n\n"+section, output, cwd)
}

func runGraphWorkspaces(cfg *config.Config, cwd, output, wsFilter string) error {
	var sb strings.Builder
	sb.WriteString("# Assay Dependency Graph\n\n")
