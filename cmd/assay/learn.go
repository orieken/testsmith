...
func newLearnCmd() *cobra.Command {
	cmd := &cobra.Command{
		Use:   "learn <test-file>",
		Short: "Extract a reusable test pattern from a file into .assay/patterns/",
...
.assay/patterns/ for human review and commit.

Requires LLM to be enabled in .assay.yaml (llm.enabled: true).

Examples:
  assay learn internal/payment/payment_test.go
  assay learn --dry-run src/services/user.test.ts`,
...
func runLearn(testFile string) error {
	...
	if dryRun {
		fmt.Printf("-- .assay/patterns/%s.md (dry-run) --\n%s\n", name, patternContent)
		return nil
	}
	...
	if _, statErr := os.Stat(destPath); statErr == nil {
		fmt.Printf("  – .assay/patterns/%s.md already exists — skipping. Delete it first to re-extract.\n", name)
		return nil
	}
	...
	fmt.Printf("  ✓ created  .assay/patterns/%s.md\n", name)
