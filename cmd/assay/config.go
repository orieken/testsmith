...
func newConfigInitCmd() *cobra.Command {
	...
	cmd := &cobra.Command{
		Use:   "init",
		Short: "Interactive wizard to create .assay.yaml",
		Long: `Walk through language detection, adapter selection, and optional LLM setup,
then write a commented .assay.yaml ready for 'assay generate'.

Examples:
  assay config init
  assay config init --lang java
  assay config init --yes          # accept all defaults, no prompts`,
	...
	cmd.Flags().StringVar(&outFile, "output", ".assay.yaml", "destination file path")
	return cmd
}
...
func runConfigInit(langFlag string, yes bool, outFile string) error {
	...
	fmt.Printf("\n  ✓ wrote  %s\n", absOut)
	fmt.Printf("\nRun 'assay generate --all' to start generating tests.\n")
	return nil
}
...
func printHeader() {
	...
	fmt.Println("Assay › Config Wizard")
	fmt.Println(strings.Repeat("─", 52))
	fmt.Println()
}
...
// renderConfigYAML produces a human-readable, commented .assay.yaml.
func renderConfigYAML(
	...
	w("# .assay.yaml — generated by 'assay config init'")
	w("# Run 'assay generate --all' to scaffold tests for every untested source file.")
	w("")
	w("# Primary language detected in this project.")
	w("language: %s", lang)
	w("")
	w("# Framework and mock library for each language.")
	w("# Run 'assay adapters list' to see all available options.")
	w("languages:")
	w("  %s:", lang)
...
values from .assay.yaml, plus the effective test adapter that would be
selected for the detected (or specified) language.

Examples:
  assay config show
  assay config show --lang java`,
		RunE: func(cmd *cobra.Command, _ []string) error {
