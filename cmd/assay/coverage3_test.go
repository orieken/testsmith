...
// assay coverage backfill / AC: runConfigInit with yes=true and dryRun=true prints plan without writing
...
// assay coverage backfill / AC: runConfigInit with yes=true and dryRun=false writes .assay.yaml
func TestRunConfigInit_YesWritesFile(t *testing.T) {
	...
	if _, err := os.Stat(filepath.Join(dir, ".assay.yaml")); err != nil {
		t.Error(".assay.yaml not written by runConfigInit(yes=true)")
	}
}

// assay coverage backfill / AC: runConfigInit auto-detects Go when no langFlag given (yes=true)
func TestRunConfigInit_AutoDetect_YesMode(t *testing.T) {
	...
	if err := runConfigInit("", true, ".assay.yaml"); err != nil {
		t.Fatalf("runConfigInit auto-detect: %v", err)
	}
}

// assay coverage backfill / AC: runConfigInit with existing file and yes=true overwrites without prompt
func TestRunConfigInit_YesOverwritesExistingFile(t *testing.T) {
	...
	mkFile(t, filepath.Join(dir, ".assay.yaml"), "# existing\n")
...
// assay coverage backfill / AC: runMigrate returns error when no migration rule exists for the given from→to pair
...
// assay coverage backfill / AC: runMigrate returns error when langFlag is invalid
...
// assay coverage backfill / AC: runValidate restricts validation to --path directory when pathFlag is set
...
// assay coverage backfill / AC: runValidateWorkspaces logs and skips workspace when resolveWorkspaceDriver fails
...
// assay coverage backfill / AC: runGraphWorkspaces logs and continues when workspace resolution fails
...
// assay coverage backfill / AC: writeGraphReport writes content to the output file when dryRun=false
...
// assay coverage backfill / AC: buildGraphSection emits node/edge count when verbose=true and label is set
...
// assay coverage backfill / AC: runGapsWorkspaces continues to next workspace when one workspace fails
...
// assay coverage backfill / AC: runPruneWorkspaces logs and continues when workspace resolution fails
...
// assay coverage backfill / AC: runGaps returns error when project detection fails (no project markers in empty dir)
func TestRunGaps_DetectError(t *testing.T) {
	dir := t.TempDir()
	testChdir(t, dir)
	dryRun = false

	// Empty dir + no .assay.yaml → config.Load returns default (no workspaces),
...
// assay coverage backfill / AC: runGraph returns error when project detection fails
...
// assay coverage backfill / AC: runPrune returns error when project detection fails
...
// assay coverage backfill / AC: writeReport prints markdown content instead of writing file in dry-run mode
...
func TestRunInit_TESTSMITHAlreadyExists_Coverage(t *testing.T) {
	...
	mkFile(t, filepath.Join(dir, "ASSAY.md"), "# existing knowledge\n")

	if err := runInit("go", false); err != nil {
		t.Fatalf("runInit ASSAY.md exists: %v", err)
	}
}

// ── runValidate non-workspace single-project auto-detect (validate.go:75) ─────

// assay coverage backfill / AC: runValidate auto-detects language when langFlag is empty and no workspaces
...
// assay coverage backfill / AC: runMigrate uses auto-detected driver when langFlag is empty
...
// assay coverage backfill / AC: runGenerate returns error when project detection fails (empty dir, no args)
...
// assay coverage backfill / AC: runGenerate returns error for unknown language flag
...
// assay coverage backfill / AC: pruneOne exits early with "no fixtures" when fixtureDir is empty
