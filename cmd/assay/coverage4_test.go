...
// assay coverage backfill / AC: runConfigInit with yes=false confirms language and writes file when user accepts
func TestRunConfigInit_Interactive_ConfirmAndWrite(t *testing.T) {
	...
	if err := runConfigInit("go", false, ".assay.yaml"); err != nil {
		t.Fatalf("runConfigInit interactive: %v", err)
	}
	if _, err := os.Stat(filepath.Join(dir, ".assay.yaml")); err != nil {
		t.Error(".assay.yaml not written")
	}
}

// assay coverage backfill / AC: runConfigInit aborts gracefully when user says no at the write prompt
...
// assay coverage backfill / AC: runConfigInit enables LLM when user answers y to the LLM prompt
...
// assay coverage backfill / AC: runConfigInit selects Anthropic when user picks provider 1
...
// assay coverage backfill / AC: runConfigInit selects OpenAI when user picks provider 2
...
// assay coverage backfill / AC: runConfigInit accepts a custom exclude-dirs list
...
// assay coverage backfill / AC: runConfigInit prompts to overwrite and aborts when user says no
func TestRunConfigInit_Interactive_OverwriteAbort(t *testing.T) {
	...
	content, _ := os.ReadFile(filepath.Join(dir, ".assay.yaml"))
	if !strings.Contains(string(content), "# existing") {
		t.Error("original file should be preserved after overwrite abort")
	}
}

// assay coverage backfill / AC: runConfigInit selects a custom adapter when user picks a non-default index
...
// assay coverage backfill / AC: runMigrate writes the migrated file to disk when dryRun=false
...
// assay coverage backfill / AC: runGenerateWorkspaces prints "no untested files" when all sources have tests
...
// assay coverage backfill / AC: runGenerateWorkspaces logs error and continues when workspace driver fails
...
// assay coverage backfill / AC: runGenerate processes explicit file arguments in single-project mode
...
// assay coverage backfill / AC: runGenerate with all=true discovers all untested files in the project
...
// assay coverage backfill / AC: runConfigInit uses pickLanguage when auto-detection fails and yes=false
func TestRunConfigInit_Interactive_PickLanguageFallback(t *testing.T) {
	...
	err := runConfigInit("", false, ".assay.yaml")
