...
""", encoding="utf-8")
    ...
    path, action = generate_or_update_fixture("stripe", ["stripe.checkout"], {}, tmp_path, config)
...
""", encoding="utf-8")
    ...
    path, action = generate_or_update_fixture("stripe", ["stripe"], {}, tmp_path, config)
    assert action == "skipped"

def test_generate_conftest():
    files = [Path("tests/fixtures/stripe_fixture.py"), Path("tests/fixtures/auth_fixture.py")]
