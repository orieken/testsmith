...
def test_cli_end_to_end(sample_project, capsys):
    ...
    with (
        patch("sys.argv", ["testsmith", str(source_file)]),
    ...
    ), f"Conftest output missing. Output:\n{captured.out}"

    # Verify Files

    # 1. Fixture
    fixture_path = (
        project_root / "tests/fixtures/requests_fixture.py"
    )  # fixture_suffix? default is ".fixture.py"?
...
def test_cli_dry_run(sample_project, capsys):
    ...
    with (
        patch("sys.argv", ["testsmith", str(source_file), "--dry-run"]),
        patch("pathlib.Path.cwd", return_value=project_root),
    ):
...
def test_cli_file_not_found(sample_project, capsys):
    ...
    with (
        patch("sys.argv", ["testsmith", "nonexistent.py"]),
        patch("pathlib.Path.cwd", return_value=project_root),
    ):
