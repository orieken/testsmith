...
def test_determine_fixture_params():
    imports = [
        ImportInfo(
            module="stripe.checkout", names=[], is_from=False, alias=None, line_number=1
        ),
        ImportInfo(
            module="requests", names=[], is_from=False, alias=None, line_number=2
        ),
    ]
...
def test_generate_test_file(config, tmp_path):
    ...
    project = ProjectContext(
        root=tmp_path, package_map={}, conftest_path=None, existing_paths=[]
    )
    ...
    imports = ClassifiedImports(
        ...
        external=[
            ImportInfo(
                module="stripe", names=[], is_from=False, alias=None, line_number=1
            )
        ],
    )
    ...
    assert (
        expected_import in content
    ), f"Expected '{expected_import}' in content:\n{content}"
