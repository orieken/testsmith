...
from testsmith.maintenance.fixture_pruner import (
    scan_used_dependencies, scan_existing_fixtures, identify_unused_fixtures,
    prune_fixtures, update_test_imports
)
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    ...
    parser.add_argument(
        "--version",
        action="version",
        version=f"testsmith {__version__}"
    )
    ...
    parser.add_argument(
        "--path",
        help="Process all untested source files in a specific directory."
    )
    ...
    parser.add_argument(
        "--prune",
        action="store_true",
        help="Identify and remove unused fixture files."
    )
...
def process_file(source_path: Path, project_context, config, args) -> dict:
    ...
    try:
        ...
        if args.dry_run:
            ...
            result["test_path"] = derive_test_path(source_path, project_context.root, config)
        ...
        external_modules = {imp.module.split(".")[0] for imp in analysis.imports.external}
        
        for dep in external_modules:
            # Gather usage info for this dependency
            relevant_imports = [imp for imp in analysis.imports.external 
...
def run(args: argparse.Namespace) -> int:
    ...
    if args.prune:
        try:
            ...
            if dry_run:
                ...
                print("\nDeleted fixtures:")
                deleted_names = [name for name, action in results if action == "deleted"]
                for name in deleted_names:
                    print(f"  ✓ {name}.fixture.py")
                
                # Update test imports
                if deleted_names:
                    modified_tests = update_test_imports(project_context.root, deleted_names)
                    if modified_tests:
    ...
    if args.coverage_gaps:
        try:
            ...
            coverage = detect_test_coverage(project_context.root, test_root_path, config)
    ...
    try:
        ...
        try:
            ...
            print("Error: Could not detect project root. Ensure a pyproject.toml exists.")
            return 1

        if args.init:
            print("Initialization not fully implemented in V1. Use 'testsmith <file>' to generate.")
            return 0
        ...
        if args.all:
            ...
            files_to_process = discover_untested_files(project_context.root, project_context.root / config.test_root, config)
        elif args.path:
            ...
            files_to_process = discover_files_in_path(target_path, project_context.root, project_context.root / config.test_root, config)
