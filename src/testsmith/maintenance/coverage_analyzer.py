...
def detect_test_coverage(project_root: Path, test_root: Path, config: TestSmithConfig) -> dict[str, str]:
    """
    ...
    for source_path in project_root.rglob("*.py"):
        ...
        if source_path.name.startswith("test_") or source_path.name == "__init__.py":
            continue
        test_root_name = test_root.name if isinstance(test_root, Path) else test_root.rstrip("/").split("/")[-1]
...
def prioritize_gaps(coverage: dict[str, str], metrics: dict[str, ModuleMetrics]) -> list[CoverageGap]:
    """
...
def generate_report(gaps: list[CoverageGap], coverage: dict[str, str]) -> str:
    ...
    lines.append(f"- **No test**: {no_test} ({no_test/total*100:.1f}%)" if total > 0 else "- **No test**: 0")
    lines.append(f"- **Skeleton only**: {skeleton} ({skeleton/total*100:.1f}%)" if total > 0 else "- **Skeleton only**: 0")
    lines.append(f"- **Partial coverage**: {partial} ({partial/total*100:.1f}%)" if total > 0 else "- **Partial coverage**: 0")
    lines.append(f"- **Fully covered**: {covered} ({covered/total*100:.1f}%)" if total > 0 else "- **Fully covered**: 0")
    lines.append("")
    ...
    lines.append("")
    lines.append("Files are prioritized by coupling (external dependencies + dependents) and coverage status.")
    lines.append("")
    lines.append("| Priority | File | Status | Ext Deps | Dependents | Suggested Command |")
    lines.append("|----------|------|--------|----------|------------|-------------------|")
    
    for i, gap in enumerate(gaps[:20], 1):  # Top 20
        rel_path = gap.source_path.name  # Just filename for brevity
        status_emoji = "❌" if gap.status == "no_test" else "⚠️" if gap.status == "skeleton_only" else "🔸"
        lines.append(
