...
def parse_paths_to_add(
    conftest_content: str, var_name: str
...
def compute_required_paths(
    source_path: Path, test_path: Path, fixture_dir: Path, project_root: Path
) -> list[str]:
    """
...
def update_conftest(
    conftest_path: Path, new_paths: list[str], config: TestSmithConfig
) -> tuple[Path, str]:
    """
    ...
    if start == -1:
        ...
        block = render_conftest_pytest_configure(
            missing
        )  # This generates variable AND logic
        ...
        if "," in line_with_bracket:
            ...
            indent = line_with_bracket[
                : len(line_with_bracket) - len(line_with_bracket.lstrip())
            ]
