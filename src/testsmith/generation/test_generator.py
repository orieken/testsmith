...
def derive_test_path(source_path: Path, project_root: Path, config: TestSmithConfig) -> Path:
    """
...
def generate_test_file(
    ...
    if names_to_import:
        internal_imports.append(f"from {module_path} import {', '.join(sorted(names_to_import))}")
