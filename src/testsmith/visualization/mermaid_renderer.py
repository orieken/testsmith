...
def render_mermaid(graph: DependencyGraph, metrics: dict[str, ModuleMetrics]) -> str:
    ...
    lines.append("    classDef mediumCoupling fill:#FFD700,stroke:#333,stroke-width:2px")
...
def render_metrics_table(metrics: dict[str, ModuleMetrics]) -> str:
    ...
    lines.append("")
    lines.append("| Module | Internal Deps | External Deps | Dependents | Coupling Score |")
    lines.append("|--------|---------------|---------------|------------|----------------|")
    
    # Sort by coupling score descending
    sorted_metrics = sorted(metrics.values(), key=lambda m: m.coupling_score, reverse=True)
    ...
    lines.append("- Higher scores indicate modules that are harder to test in isolation")
    lines.append("")
